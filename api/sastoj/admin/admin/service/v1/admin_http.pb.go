...
const OperationAdminGetMQQueueStats = "/api.sastoj.admin.admin.service.v1.Admin/GetMQQueueStats"
...
func RegisterAdminHTTPServer(s *http.Server, srv AdminHTTPServer) {
	...
	r.POST("/contest/{contest_id}/rank", _Admin_ManualRanking0_HTTP_Handler(srv))
	r.GET("/contest/{contest_id}/rank", _Admin_GetRanking0_HTTP_Handler(srv))
	...
	r.GET("/mq/queue-stats", _Admin_GetMQQueueStats0_HTTP_Handler(srv))
}
...
func _Admin_GetMQQueueStats0_HTTP_Handler(srv AdminHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetMQQueueStatsRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationAdminGetMQQueueStats)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetMQQueueStats(ctx, req.(*GetMQQueueStatsRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetMQQueueStatsReply)
		return ctx.Result(200, reply)
	}
}
...
type AdminHTTPClient interface {
	...
	GetMQQueueStats(ctx context.Context, req *GetMQQueueStatsRequest, opts ...http.CallOption) (rsp *GetMQQueueStatsReply, err error)
...
func (c *AdminHTTPClientImpl) GetMQQueueStats(ctx context.Context, in *GetMQQueueStatsRequest, opts ...http.CallOption) (*GetMQQueueStatsReply, error) {
	var out GetMQQueueStatsReply
	pattern := "/mq/queue-stats"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationAdminGetMQQueueStats))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
...
func (c *AdminHTTPClientImpl) GetRanking(ctx context.Context, in *GetRankingRequest, opts ...http.CallOption) (*GetRankingReply, error) {
	...
	pattern := "/contest/{contest_id}/rank"
	path := binding.EncodeURL(pattern, in, true)
