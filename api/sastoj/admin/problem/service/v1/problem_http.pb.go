...
const OperationProblemGetProblemTypes = "/api.sastoj.admin.problem.service.v1.Problem/GetProblemTypes"
...
func RegisterProblemHTTPServer(s *http.Server, srv ProblemHTTPServer) {
	...
	r.GET("/problem-types", _Problem_GetProblemTypes0_HTTP_Handler(srv))
}
...
func _Problem_GetProblemTypes0_HTTP_Handler(srv ProblemHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetProblemTypesRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationProblemGetProblemTypes)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
...
func (c *ProblemHTTPClientImpl) GetProblemTypes(ctx context.Context, in *GetProblemTypesRequest, opts ...http.CallOption) (*GetProblemTypesReply, error) {
	var out GetProblemTypesReply
	pattern := "/problem-types"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationProblemGetProblemTypes))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
